...
% Mp is 0.
Mp = 100*(MaxResponse - ssVal)/ssVal; %here we calculate Mp percentage
if Mp < 0                             %if this is negative set it to 0
    Mp = 0;
end
...
while (y(x)<1.05*ssVal)&&(y(x)>.95*ssVal)   %(y is within 5% of ssVal)
    x = x-1;
end
...
% document tss
text(t(tssIndex),y(tssIndex),'\leftarrow Steady-State',...
    'HorizontalAlignment','left')
